...
export function CoachPaymentDetailClient({ id }: { id: string }) {
  ...
  const receiptNumber = `RS-${payment.dueDate.replace('-', '')}-${payment.id.substring(0, 4).toUpperCase()}`;
        ...
        <div ref={receiptRef} className="p-12 bg-white text-black min-h-[297mm] flex flex-col font-sans">
          <header className="flex justify-between border-b-2 border-black pb-8 mb-10 bg-white">
            <div className="flex items-center gap-6 bg-white">
              ...
              <div className="bg-white">
                ...
                <p className="bg-white text-sm font-semibold tracking-widest text-muted-foreground uppercase">Bulletin de Salaire</p>
              </div>
            </div>
          ...
          <div className="flex justify-between items-start mb-12 bg-white">
            <div className="bg-white">
              ...
              <p className="text-lg text-muted-foreground font-medium bg-white">Salaire: {payment.dueDate}</p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-8 p-10 border-2 border-black rounded-2xl mb-12 bg-white">
            <div className="flex justify-between text-2xl bg-white"><span>Total Net</span><span className="font-bold bg-white">{payment.totalAmount.toFixed(2)} DH</span></div>
            <div className="flex justify-between text-2xl bg-white"><span>Réglé</span><span className="font-bold text-green-600 bg-white">{payment.paidAmount.toFixed(2)} DH</span></div>
