...
export function PlayerDetailClient({ id }: { id: string }) {
  ...
  if (loading && !isSubmitting) return <div className="p-8"><Skeleton className="h-[600px] w-full rounded-[40px]" /></div>;
  ...
  return (
    <div className="flex-1 space-y-6 p-4 md:p-8 pt-6 bg-background">
      ...
      <Card className="overflow-hidden border shadow-xl rounded-[40px] bg-card">
        <CardHeader className="flex flex-col md:flex-row items-center md:items-start gap-8 bg-primary/5 p-10">
          <div className="relative">
            <Avatar className="h-48 w-48 border-4 border-background shadow-2xl rounded-3xl">
            ...
            <Badge className="absolute -bottom-4 left-1/2 -translate-x-1/2 px-6 py-2 text-sm font-black uppercase tracking-widest shadow-xl border-4 border-background">
              {player.status}
            </Badge>
          </div>
        ...
        <CardContent className="p-10 space-y-12">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
            ...
            <div className="space-y-6">
              <h3 className="flex items-center gap-3 font-black text-primary border-b-2 border-primary/10 pb-3 uppercase text-xs tracking-widest italic"><User className="size-5" /> Identité Officielle</h3>
              <div className="grid gap-4">
                ...
                <div className="flex justify-between items-center bg-muted/10 p-3 rounded-xl"><span className="text-muted-foreground text-xs font-bold uppercase">À</span> <span className="font-black">{player.birthPlace || 'N/A'}</span></div>
            ...
            <div className="space-y-6">
              <h3 className="flex items-center gap-3 font-black text-primary border-b-2 border-primary/10 pb-3 uppercase text-xs tracking-widest italic"><Trophy className="size-5" /> Profil Terrain</h3>
              <div className="grid gap-4">
                ...
                <div className="flex justify-between items-center bg-muted/10 p-3 rounded-xl"><span className="text-muted-foreground text-xs font-bold uppercase">Poste</span> <Badge className="font-black uppercase">{player.poste}</Badge></div>
            ...
            <div className="space-y-6">
              <h3 className="flex items-center gap-3 font-black text-primary border-b-2 border-primary/10 pb-3 uppercase text-xs tracking-widest italic"><HeartPulse className="size-5" /> Suivi Santé</h3>
            ...
            <div className="space-y-6">
              <h3 className="flex items-center gap-3 font-black text-primary border-b-2 border-primary/10 pb-3 uppercase text-xs tracking-widest italic"><Banknote className="size-5" /> Trésorerie</h3>
              <div className="grid gap-4">
                <div className="flex justify-between items-center bg-muted/10 p-3 rounded-xl">
                ...
                <div className="flex flex-col items-center bg-primary p-6 rounded-[32px] text-white shadow-2xl">
                  ...
                  <span className="text-4xl font-black italic">{player.subscriptionAmount} <small className="text-base uppercase">DH</small></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <CardFooter className="justify-end gap-4 border-t bg-muted/5 p-8">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent className="rounded-[32px]">
              <AlertDialogHeader>
                <AlertDialogTitle className="text-2xl font-black uppercase text-destructive italic">Radiation de l'effectif ?</AlertDialogTitle>
                <AlertDialogDescription className="font-bold text-sm">
                  Cette opération est irréversible. Le matricule <span className="text-primary">{player.professionalId}</span> sera désactivé.
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0 overflow-hidden">
                  ...
                  <div className="flex flex-col items-center gap-6 bg-primary/5 p-8 rounded-[32px] border-2 border-dashed border-primary/20">
                    ...
                    <FormField control={form.control} name="photo" render={({field}) => (
                      <FormItem className="w-full max-w-md">
                        <FormLabel className="font-black text-center block text-primary uppercase text-[10px] tracking-widest">URL de la Photo</FormLabel>
              ...
              <DialogFooter className="p-8 border-t bg-muted/10 flex gap-4">
                <Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={isSubmitting} className="h-14 px-8 rounded-2xl font-black uppercase text-xs">Annuler</Button>
                <Button type="submit" disabled={isSubmitting} className="h-14 flex-1 rounded-2xl font-black uppercase text-sm shadow-xl text-white bg-primary hover:bg-primary/90">
                  {isSubmitting ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : "Mettre à jour la fiche"}
                </Button>
              </DialogFooter>
            </form>
          </Form>
