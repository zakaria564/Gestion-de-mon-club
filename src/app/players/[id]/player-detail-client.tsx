...
import { ArrowLeft, Cake, Edit, Trash2, Camera, Home, Shirt, Phone, Flag, Shield, Mail, MapPin, PlusCircle, X, VenetianMask, UserSquare, UserCheck, ExternalLink, FileText } from "lucide-react";
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center gap-6">
          <Avatar className="h-32 w-32 border"><AvatarImage src={player.photo} /><AvatarFallback className="text-4xl">{player.name.substring(0, 2)}</AvatarFallback></Avatar>
          <div className="flex-1"><CardTitle className="text-3xl font-bold">{player.name}</CardTitle></div>
        </CardHeader>
        <CardContent className="pt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3"><Home className="h-4 w-4 text-muted-foreground" /><Badge style={{ backgroundColor: categoryColors[player.category.replace(' F', '')] }}>{player.category}</Badge></div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {player.documents.map((d, i) => (
                  <div key={i} className="flex items-center justify-between p-3 border rounded-lg bg-muted/10">
                    <div className="flex items-center gap-2"><FileText className="h-4 w-4" /><span className="text-sm font-medium">{d.name}</span></div>
                    <Button variant="ghost" size="icon" asChild><a href={d.url} target="_blank"><ExternalLink className="h-4 w-4" /></a></Button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <CardFooter className="justify-end gap-2 border-t pt-6">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Confirmer ?</AlertDialogTitle><AlertDialogDescription>Action irréversible.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Annuler</AlertDialogCancel><AlertDialogAction onClick={async () => { await deletePlayer(id); router.push('/players'); }}>Supprimer</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
          </AlertDialog>
        </CardFooter>
      </Card>
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      ...
                      <FormField control={form.control} name="birthDate" render={({field}) => <FormItem><FormLabel>Date naissance</FormLabel><Input type="date" {...field} required /></FormItem>} />
