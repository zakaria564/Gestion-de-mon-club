...
function PlayersContent() {
  ...
  const form = useForm<PlayerFormValues>({
    ...
    defaultValues: {
      name: '', birthDate: '', address: '', phone: '', email: '', country: 'Marocaine', poste: '', jerseyNumber: 0, photo: '', cin: '', tutorName: '', tutorPhone: '', tutorEmail: '', tutorCin: '', status: 'Actif', category: '', gender: 'Masculin', entryDate: '', exitDate: '', coachName: '', documents: [],
    },
  });
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return players.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.poste.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [players, searchQuery]);
      ...
      <Tabs value={activeGender} onValueChange={(v) => handleTabChange('gender', v)} className="w-full">
        ...
        <TabsContent value={activeGender}>
          {availableCategories.length > 0 ? (
            <Tabs value={currentCategory} onValueChange={(v) => handleTabChange('category', v)} className="w-full mt-4">
              <TabsList className="h-auto p-1 bg-muted rounded-md flex-wrap justify-start">
                {availableCategories.map(cat => (
                  <TabsTrigger key={cat} value={cat} style={{ backgroundColor: categoryColors[cat] }} className="text-white data-[state=active]:brightness-110">
                    {cat}
                  </TabsTrigger>
                ))}
              </TabsList>
                  ...
                  <div className="space-y-4 pb-10">
                    ...
                    {fields.map((field, index) => (
                      <div key={field.id} className="p-4 border rounded-md space-y-4 relative bg-muted/20">
                        ...
                        <FormField control={form.control} name={`documents.${index}.name`} render={({field}) => <FormItem><FormLabel>Nom du doc</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>{documentOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></FormItem>} />
...
export default function PlayersPage() {
  return <Suspense fallback={<div className="p-8">Chargement...</div>}><PlayersContent /></Suspense>;
}